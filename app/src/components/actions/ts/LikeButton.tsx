...
const LikeButton: React.FC<LikeButtonProps> = (props) => {
    ...
    return (
        <div className="flex items-center justify-center w-full select-none m-0">
            <div className="w-fit">
                {!actionData?.like?.status && <HeartOutLine className="m-0 cursor-pointer sm:text-[30px] xs:text-[1.5rem] lg:text-[1.5rem]" onClick={() => like(true)} />}
                {actionData?.like?.status && <HeartFilled className="m-0 cursor-pointer text-[red] sm:text-[30px] xs:text-[1.5rem] lg:text-[1.5rem]" onClick={() => like(false)} />}
                <p className="w-full mt-2 lg:text-[0.7rem] text-center sm:text-[1rem] xs:text-[0.7rem]">{formatNumber(actionData?.like?.count)}</p>
            </div>
        </div>
    )
}
