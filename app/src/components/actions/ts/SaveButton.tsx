...
const SaveButton: React.FC<SaveButtonProps> = (props) => {
    ...
    return (
        <div className="flex items-center justify-center w-full select-none">
            <div className="w-fit">
                {!actionData?.save && <SaveOutline className="m-0 cursor-pointer sm:text-[30px] xs:text-[1.5rem] lg:text-[1.5rem]" onClick={()=>save(true)} />}
                {actionData?.save && <SaveFilled className="m-0 cursor-pointer sm:text-[30px] xs:text-[1.5rem] lg:text-[1.5rem]" onClick={()=>save(false)}/>}
            </div>
        </div>
    )
}
